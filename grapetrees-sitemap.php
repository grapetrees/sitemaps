...
Plugin URI:  https://github.com/grapetrees/sitemaps
Description: Display Html Sitemaps
Version: 1.0.0
Author: Grapetrees
Author URI: https://github.com/grapetrees/
Text Domain: grapetrees-sitemap
License: GPLv3 or later
*/

function grapetrees_sitemap_cat($categoryid = 0, $returnhtml = null) {
	...
	if ( (is_array($categories)) &&  (count ( $categories ) > 0)) 
	{
		foreach ( $categories as $categorie_single ) {
			...
			$returnhtml = grapetrees_sitemap_cat ( $categorie_single->term_id, $returnhtml );
			$returnhtml .= "</ul>";
		}
	}
...
function grapetrees_sitemap() {
	$returnhtml = '';
	$returnhtml .= grapetrees_sitemap_cat ( 0, $returnhtml );
...
add_shortcode ( 'sitemappage', 'grapetrees_sitemap' );
